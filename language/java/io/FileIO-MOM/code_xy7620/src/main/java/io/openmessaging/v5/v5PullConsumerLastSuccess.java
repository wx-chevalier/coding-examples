 ...
 */
public class v5PullConsumerLastSuccess implements PullConsumer {
	...
	static HashMap<String,String> allQueueOrTopic = new HashMap<String,String>(256);
...
//	Method getCleanerMethod = ByteBuffer.getClass().getMethod("cleaner", new Class[0]);
	...
	public v5PullConsumerLastSuccess(KeyValue properties) throws IOException {
...
//		System.out.println(Thread.currentThread().getName()+"读文件 第"+readFileTimes+++" 次");
		try {
			...
			try {
				...
				System.out.println(Thread.currentThread().getName()+" BufferUnderflowException 读完了 ");
				e.printStackTrace();
				return false;
			}
...
//						System.out.println(Thread.currentThread().getName()+" header = 0 本轮完毕");
				...
				for (int i = 0; i < headerSize; i++) {
					 ...
					 */
					if (seq == 2 ) {
						queueOrTopic = allQueueOrTopic.get(value);
						value = queueOrTopic;
					}else if(seq == 1){
						queueOrTopic = allQueueOrTopic.get(value);
						value = queueOrTopic;
						//如果data没有topic，说明该对象不需要保存
						isTopicNeeded = data.containsKey(queueOrTopic);
					}
